...
const getConnectorMeta = async (connectorName, connectorCountry) => {
  ...
  for (const conn of (
    await metaApi.getConnectors({ country: connectorCountry })
  ).connectors) {
...
async function main() {
  ...
  if (authResponse.url) {
    ...
    for (let i = 0; i < 10; i++) {
      try {
        makeTokenResponse = await authApi.makeToken("authorization_code", "", {
          authEnv: authResponse.env,
        });
        break;
      } catch (err) {
  ...
  if (apiMeta.modifyConsentsInfo[0].info.accountsRequired) {
    const accountIds = getAccountsResult.accounts.map(
      (el) =>
        new enablebanking.AccountIdentification({ iban: el.accountId.iban })
    );
